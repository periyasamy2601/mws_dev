...
import 'package:avk/packages/mobile_phone_field_pacakage/src/country_picker_dialog.dart';
import 'package:avk/packages/mobile_phone_field_pacakage/src/phone_number.dart';
import 'package:flutter/foundation.dart' show kIsWeb;
import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

/// mws phone field package
class MwsPhoneField extends StatefulWidget {
  ...
  const MwsPhoneField(
      {super.key,
        ...
        this.showMaxDialLength = true,
        this.pickerType = PickerType.dialog});
  ...
  /// {@macro flutter.auth_widgets.editableText.readOnly}
  final bool readOnly;

  /// on saved
  final FormFieldSetter<PhoneNumber>? onSaved;

  /// {@macro flutter.auth_widgets.editableText.onChanged}
  ///
  /// See also:
  ///
  ...
  /// {@macro flutter.auth_widgets.editableText.keyboardType}
  ...
  /// {@macro flutter.auth_widgets.editableText.onSubmitted}
  ///
  /// See also:
  ///
  ...
  /// custom drop down decoration class
  final BoxDecoration dropdownDecoration;

  /// The style use for the country dial code.
  final TextStyle? dropdownTextStyle;

  /// {@macro flutter.auth_widgets.editableText.inputFormatters}
  ...
  /// show country max-dial-length based on country selection
  final bool showMaxDialLength;

  /// If null, default magnification configuration will be used.
  final TextMagnifierConfiguration? magnifierConfiguration;

  /// show to picker based on picker type
  final PickerType pickerType;

  @override
  State<MwsPhoneField> createState() => _MwsPhoneFieldState();
}

class _MwsPhoneFieldState extends State<MwsPhoneField> {
  ...
  void initState() {
    ...
    if (widget.initialCountryCode == null && number.startsWith('+')) {
      ...
      _selectedCountry = countries.firstWhere(
              (Country country) => number.startsWith(country.fullCountryCode),
      ...
      _selectedCountry = _countryList.firstWhere(
              (Country item) => item.code == (widget.initialCountryCode ?? 'US'),
          orElse: () => _countryList.first);
    ...
    if (widget.autoValidateMode == AutovalidateMode.always) {
      ...
      final String? value = widget.validator?.call(initialPhoneNumber.number);
  ...
  Future<void> _changeCountry() async {
    ...
    widget.pickerType == PickerType.dialog
      ...
      builder: (BuildContext context) => StatefulBuilder(
        builder: (BuildContext ctx,void Function(void Function()) setState) => _countryPickerDialog(setState),
      ),
    )
        : await showModalBottomSheet(
      context: context,
      constraints: BoxConstraints(maxWidth: widget.pickerDialogStyle?.width ?? 640),
      builder: (BuildContext context) => StatefulBuilder(
        builder: (BuildContext ctx, void Function(void Function()) setState) => _countryPickerDialog(setState),
      ),
    );
  ...
  CountryPickerDialog _countryPickerDialog(StateSetter setState) {
  ...
  Widget build(BuildContext context) {
    return TextFormField(
      ...
      decoration: widget.decoration.copyWith(
        ...
        counterText: widget.showMaxDialLength ? null : '',
      ),
      style: widget.style,
      ...
      validator: widget.validator ??
              (String? val) {
            if (val?.length != _selectedCountry.maxLength) {
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                if (widget.enabled &&
                    ...
                    widget.dropdownIconPosition == IconPosition.leading) ...<Widget>[
                  widget.dropdownIcon,
                  const SizedBox(width: 4),
                ],
                if (widget.showCountryFlag) ...<Widget>[
                  if (kIsWeb) Image.asset(
                ...
                if (widget.enabled &&
                    widget.showDropdownIcon &&
                    widget.dropdownIconPosition == IconPosition.trailing) ...<Widget>[
                  const SizedBox(width: 4),
                  widget.dropdownIcon,
                ],
                const SizedBox(width: 8),
...
/// Defines the position of an icon relative to its content.
enum IconPosition {
  /// Icon appears before the text/content.
  leading,

  /// Icon appears after the text/content.
  trailing,
}

/// Defines the style of picker to be used.
enum PickerType {
  /// Picker is displayed in a modal dialog.
  dialog,

  /// Picker is displayed as a bottom sheet.
  bottomSheet,
}
